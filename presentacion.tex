...
\frame{
	...
	\begin{block}{Generación de puntos realistas en el mapa}
		...
		\item Instancias en Montevideo generadas manualmente.\pause
	...
	\alert{22} instancias para el PVCT monoobjetivo y \alert{88} para el multiobjetivo, considerando distintas capacidades y tolerancias.
...
\frame{
	...
	\begin{itemize}
		\item \textbf{seqEA}: 20 ejecuciones de 2000 generaciones sobre 3 instancias.\\{\small $\text{\#\textit{P}} \in \lbrace 150, 200, \alert{250}\rbrace$; $p_C \in \lbrace \alert{0.6}, 0.75, 0.95\rbrace$; $p_M \in \lbrace 0.001, 0.01, \alert{0.1}\rbrace$.}\pause	
...
\frame{
	...
	\begin{block}{Metodología}
		\begin{itemize}
			\item 30 ejecuciones independientes de 10.000 generaciones cada una.
			\item Los resultados no siguen una distribución normal (Shapiro--Wilk, $\alpha=0.05$).
	...
	\begin{block}{\textit{seqEA}: inicialización aleatoria vs. incialización ávida}
	...
	\begin{block}{\textit{p$\mu$EA}: inicialización aleatoria vs. incialización ávida}
		La inicialización ávida supera la inicialización aleatoria en \alert{11} instancias de prueba. No hay instancias en las que se pueda afirmar que la inicialización aleatoria alcance mejores soluciones.
	\end{block}
}

\frame{
	\frametitle{Mejoras \textit{seqEA} sobre algoritmo ávido}
	Se alcanzaron mejoras en \textbf{todas} las instancias. En el mejor caso se superó el costo del algoritmo ávido en un \alert{35.9\%}.
	\input{./evaluacion_experimental/mejoras_greedy_clei/imagenes.tex}
}

\frame{
	\frametitle{Mejoras \textit{p$\mu$EA} sobre algoritmo ávido}
	Se alcanzaron mejoras en \textbf{todas} las instancias. En el mejor caso se superó el costo del algoritmo ávido en un \alert{41.0\%}.
	\input{./evaluacion_experimental/mejoras_greedy_alio/imagenes.tex}
}

\frame{
	\frametitle{Comparativa \textit{seqEA} vs. \textit{p$\mu$EA}}
	\textit{p$\mu$EA} supera a \textit{seqEA} en \alert{17 de 22} instancas. Únicamente en \alert{1} instancia \textit{seqEA} supera a \textit{p$\mu$EA} (Kruskal--Wallis, $\alpha=0.05$).
...
\frame{
	...
	\begin{block}{Configuración paramétrica}
		\begin{itemize}
			\item \textbf{p$\mu$MOEA/D}: 30 ejecuciones de 20000 generaciones sobre 4 instancias. \\{\small $P=15$; $p_C \in \lbrace 0.6, \alert{0.75}, 0.95\rbrace$; $p_M \in \lbrace 0.001, 0.01, \alert{0.1}\rbrace$; migración cada 1000 generaciones con selección por torneo ($m=2,k=1$).}\\
...
\frame{
	...
	\begin{block}{\textit{p$\mu$MOEA/D}}
		Buena convergencia y diversidad. La cantidad de soluciones no dominadas es baja.
	...
	\begin{block}{\textit{NSGA--II}}
		Mayor cantidad de puntos no dominados (hasta 67/80). Buena convergencia y diversidad en las soluciones encontradas.	
...
\frame{
	...
	\alert{\textit{NSGA--II} alcanza mejores soluciones}: mayor cantidad de puntos no dominados distribuidos homogéneamente.
...
\frame{
	...
	\textit{NSGA--II} alcanza mejores soluciones pero requiere de un mayor tiempo de ejecución que \textit{p$\mu$MOEA/D}.
