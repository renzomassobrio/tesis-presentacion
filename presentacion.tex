...
\frame{
	...
	\begin{block}{Definición}
		\begin{itemize}
			\item Técnicas estocásticas que emulan el proceso de evolución natural de las especies.
			\item Aplicadas a problemas de \alert{optimización}, búsqueda y aprendizaje.
			\item Técnica iterativa (\textbf{generación}) que aplica operadores estocásticos sobre un conjunto de individuos (\textbf{población}).
			\item Cada individuo codifica una solución tentativa al problema y tiene un valor de \textbf{fitness}.
			\item El propósito es mejorar el fitness de los individuos en la población mediante la aplicación de \textbf{operadores evolutivos}.
...
\frame{
	...
	\begin{block}{Micro AE paralelo (\textit{p$\mu$EA})}
		\begin{itemize}
			\item Se busca \textbf{mejorar el desempe\~no} mediante el paralelismo.
...
\frame{
	...
	\begin{block}{Aspectos comunes}
		\begin{itemize}
			\item MOEA: acercarse al frente de Pareto del problema (\textbf{convergencia}) y muestrear adecuadamente el frente de soluciones (\textbf{diversidad}).
...
\frame{
	...
	\small{\textit{``Online taxi sharing optimization using evolutionary algorithms'' (CLEI)}}
...
\vspace{0.3cm}
\centering
	\textit{p$\mu$EA} supera a \textit{seqEA} en \alert{17 de 22} instancas.

	Únicamente en \alert{1} instancia \textit{seqEA} supera a \textit{p$\mu$EA}.
...
\frame{
	...
	\begin{block}{\scriptsize{\textit{p$\mu$MOEA/D: \scalebox{.7}{``Planificación multiobjetivo de viajes compartidos en taxis utilizando un micro algoritmo evolutivo paralelo'' (MAEB)}}}}
	...
	\end{block}
	%\pause
	\begin{block}{\scriptsize{\textit{NSGA--II: \tiny{``Multiobjective taxi sharing optimization using the NSGA-II evolutionary algorithm'' (MIC)}}}}
...
\frame{
	...
	\includegraphics[width=.3\textwidth]{logo-mepaseaste.png}
	\vspace{.5cm}
