...
\frame{
	...
	\end{itemize}
	Cuatro variantes de cada instancias para el PVCT multiobjetivo considerando distintas capacidades y tolerancias.
	\end{block}
...
\frame{
	...
	\begin{block}{Entorno de ejecución}
		...
		\item \textit{seqEA}: Dell Power Edge 2950, \textbf{1 núcleo} de Intel Xeon E5430 2.66GHz, 8GB RAM.
		\item \textit{p$mu$EA}: HP Proliant DL585, \textbf{24 núcleos} de AMD Opteron 2.09GHz, 48GB RAM.
		\end{itemize}
	\end{block}
...
\frame{
	...
	Se alcanzaron mejores valores en \textbf{todas} las instancias. En el mejor caso se superó el costo del algoritmo ávido en un \alert{41\%}.
...
\frame{
	...
	\begin{itemize}
	\item Se relevó la literatura relacionada (CPP, DARP, TPP) y se presentaron dos variantes del problema.
	\item Se implementaron \textbf{cuatro} AE: dos para cada variante del problema.
	\item El análisis experimental se realizó sobre instancias realistas.
	\item Los AE implementados fueron comparados contra algoritmos ávidos.
	\item Variante monoobjetivo: mejoras de hasta 35.9\% (seqEA) y 41.0\% (p$\mu$EA) sobre algoritmo ávido.
	\item Variante multiobjetivo: mejoras de hasta 72.8\% y 101.2\% (p$\mu$MOEA/D); 105.2\% y 75.1\% (NSGA--II) en costo y demora sobre algoritmos ávidos.

	\item Se implementó el planificador de viajes compartidos en taxis, disponible públicamente en \url{www.mepaseaste.uy}.
	\item Se publicaron \textbf{cuatro} artículos en conferencias internacionales.
