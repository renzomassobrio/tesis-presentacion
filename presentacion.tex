...
\frame{
	...
	\pause
		\begin{itemize}
		\item Se busca minimizar simultáneamente el \alert{costo total} y la \alert{demora total}.\pause
		\item Se incorporan dos variables:\pause
		\begin{itemize}
			\item Cada pasajero tiene un \alert{``nivel de apuro''} asociado.\pause
...
\frame{
	...
	\begin{block}{AE Paralelo (\textit{p$\mu$EA})}
		\begin{itemize}
%			\item Modelos paralelos.
%			\begin{itemize}
...
%			\column{0.4\textwidth}
				\centering
				\only<1-5>{\mbox{\phantom{\includegraphics[width=0.5\textwidth]{migracion.png}}}}
				\includegraphics<6->[width=0.5\textwidth]{migracion.png}
%			\end{columns}
%		\end{itemize}
	\end{block}
}
...
\frame{
	...
	\begin{block}{Aspectos comunes}
		\begin{itemize}
			\item Propósitos en AEs multiobjetivos:
			\begin{itemize}
				\item Acercarse al frente de Pareto del problema (\textbf{convergencia}).
				\item Muestrear adecuadamente el frente de soluciones (\textbf{diversidad}).
			\end{itemize}\pause
	...
	\column{0.65\textwidth}
		\begin{block}{AE Multiobjetivo Distribuido (\textit{p$\mu$MOEA/D})}
		%Micro--poblaciones distribuidas con
		Agregación lineal de los objetivos:
		$F = w_C\times CT + w_D \times DT$, \small \\\hspace{1cm}{\tiny$w_C = [0:\frac{1}{\#islas}:1]$, $w_D = 1 - w_C$}.
		\end{block}
	\column{0.35\textwidth}
		\centering
		\only<1-3>{\mbox{\phantom{\includegraphics[width=0.75\textwidth]{pmuMOEAD.png}}}}
		\includegraphics<4->[width=0.75\textwidth]{pmuMOEAD.png}
	\end{columns}
	\pause

	\begin{block}{AE Multiobjetivo Explícito (\textit{NSGA--II})}
		\begin{itemize}
			\item Ordenamiento no--dominado (elitista).
			\item \textit{Crowding} para preservar diversidad.
		\end{itemize}
	\end{block}
}
...
\frame{
	...
	Se alcanzaron mejoras en \textbf{todas} las instancias. En el mejor caso se mejoró el costo del algoritmo ávido en un \alert{35.9\%} (instancia grandes \#4).
...
\frame{
	...
	Se alcanzaron mejoras en \textbf{todas} las instancias. En el mejor caso se mejoró el costo del algoritmo ávido en un \alert{41.0\%} (instancia grandes \#4).
