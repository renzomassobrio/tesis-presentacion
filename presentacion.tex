...
\frame{
	...
	\begin{block}{Definición}
		Técnicas estocásticas que emulan el proceso de evolución natural de las especies para resolver problemas de optimización, búsqueda y aprendizaje.
	\end{block}
	\pause
	\begin{block}{Modelos paralelos en AE}
		Buscan \textbf{mejorar el desempe\~no} de los AE.\\
		\alert{Modelo de subpoblaciones distribuidas}: divide la poblaci\'on en \textbf{islas} que ejecutan un AE secuencial e intercambian individuos mediante \textbf{migración}.
	\end{block}
	\pause
	...
	\column{0.6\textwidth}
			\begin{block}{AE paralelo con micro--población (\textit{p$\mu$EA})}
				...
				\textit{p$\mu$EA} hace uso de \textbf{poblaciones pequeñas} e incluye un \textbf{operador específico de diversidad}.% para mitigar este problema.
			\end{block}
	\column{0.4\textwidth}
		\centering
		\only<1>{\mbox{\phantom{\includegraphics[width=0.8\textwidth]{migracion.png}}}}%
...
\frame{
	...
	\column{0.4\textwidth}
		\centering
		\includegraphics[width=.6\textwidth]{paretto.png}
	\end{columns}
	...
	\column{0.6\textwidth}
		\begin{block}{\textit{p$\mu$MOEA/D}}
				...
				\item agregación lineal de los objetivos
				\end{itemize}
		\end{block}
	\column{0.4\textwidth}
		\centering
		\only<1>{\mbox{\phantom{\includegraphics[width=0.6\textwidth]{pmuMOEAD.png}}}}%
		\includegraphics<2->[width=0.6\textwidth]{pmuMOEAD.png}
	...
	\column{0.6\textwidth}
		\begin{block}{\textit{NSGA--II}}
				\begin{itemize}
					\item ordenamiento no--dominado y elitista
					\item \textit{crowding} para preservar la diversidad
				\end{itemize}
		\end{block}
	\column{0.4\textwidth}
		\centering
		\only<1,2>{\mbox{\phantom{\includegraphics[width=0.6\textwidth]{nsga_rangos.png}}}}%
		\includegraphics<3->[width=0.6\textwidth]{nsga_rangos.png}
		%\includegraphics[width=.8\textwidth]{pmuMOEAD.png}
	\end{columns}
}
...
\frame{
	...
	Cordeau et al. (2003) \textbf{DARP estático con ventanas de tiempo}. \\\hspace{1cm} {\small Búsqueda tabú con tiempos de ejecución de hasta 90 minutos.}
	\end{block}


	\pause
	\begin{block}{Taxi pooling problem (TPP)}
	Tao et al. (2007) Heurísticas ávidas para \textbf{one--to--many} y many--to--one. \\ \hspace{1cm}{\small Las mejoras se reportan en términos absolutos.}

	Ma et al. (2013) TPP dinámico con pedidos en tiempo real. 13\% de ahorro en distancia con un algoritmo ávido en \textbf{instancias realistas}.\pause
	\end{block}
	\pause
	\begin{block}{Resumen}
...
\frame{
	...
	\column{0.43\linewidth}
		...
		\begin{itemize}
		\item Tuplas de largo $2N-1$ siendo $N$ la cantidad de pasajeros.
		\pause
		
		\item Inicialización: aleatoria y ávida.
		\pause

		\item Función correctiva: desplaza ceros para romper secuencias de dígitos inválidas.
		\pause
	...
	\column{0.57\linewidth}
		...
		\only<1,2>{\mbox{\phantom{\includegraphics[width=0.4\textwidth]{temporal.png}}}}%
		\includegraphics<3->[width=0.4\textwidth]{temporal.png}

		\only<1-3>{\mbox{\phantom{\includegraphics[width=0.7\textwidth]{pbx_simple.png}}}}%
		\includegraphics<4->[width=0.7\textwidth]{pbx_simple.png}

		\only<1-4>{\mbox{\phantom{\includegraphics[width=0.6\textwidth]{em.png}}}}%
		\includegraphics<5->[width=0.6\textwidth]{em.png}
	\end{columns}
